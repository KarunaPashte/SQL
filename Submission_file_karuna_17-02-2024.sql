...
case when p.product_price > 20000 then (p.product_price - (p.product_price*0.20))
	 when p.product_price > 10000 and  p.product_price <= 200000 then (p.product_price - (p.product_price*0.15))
     when p.product_price <= 10000 then (p.product_price - (p.product_price*0.10)) end as New_Price
from product p
...
GROUP BY 
    p.product_class_code, pc.product_class_desc
HAVING 
...
SELECT 
    a.order_id, 
    SUM(a.Biggest_Order) AS Volume_of_biggest_order 
FROM
    (SELECT 
         oi.order_id, 
         p.len * p.width * p.height * oi.product_quantity AS Biggest_Order
     FROM 
         product p
     INNER JOIN 
         order_items oi ON p.product_id = oi.product_id) AS a
GROUP BY 
    a.order_id
HAVING 
    SUM(a.Biggest_Order) <= (SELECT len * width * height FROM carton WHERE carton_id = 10)
ORDER BY 
    Volume_of_biggest_order DESC
LIMIT 1;
...
SELECT 
    a.*, 
    CASE 
        WHEN a.product_class_desc = 'Electronics' OR a.product_class_desc = 'Computer' THEN
            CASE 
                WHEN a.Quantity_sold = 0 THEN 'No Sales in past, give discount to reduce inventory'
                WHEN a.product_quantity_avail < a.Quantity_sold * 0.10 THEN 'Low inventory, need to add inventory'
                WHEN a.Quantity_sold * 0.10 <= a.product_quantity_avail AND a.product_quantity_avail < a.Quantity_sold * 0.50 THEN 'Medium inventory, need to add some inventory'
                WHEN a.product_quantity_avail >= a.Quantity_sold * 0.50 THEN 'Sufficient inventory'  
            END
        WHEN a.product_class_desc = 'Mobiles' OR a.product_class_desc = 'Watches' THEN
            CASE 
                ...
                WHEN a.Quantity_sold * 0.20 <= a.product_quantity_avail AND a.product_quantity_avail < a.Quantity_sold * 0.60 THEN 'Medium inventory, need to add some inventory'
                WHEN a.product_quantity_avail >= a.Quantity_sold * 0.60 THEN 'Sufficient inventory' 
            END
        ELSE 
            CASE 
                WHEN a.Quantity_sold = 0 THEN 'No Sales in past, give discount to reduce inventory'
                WHEN a.product_quantity_avail < a.Quantity_sold * 0.30 THEN 'Low inventory, need to add inventory'
                WHEN a.Quantity_sold * 0.30 <= a.product_quantity_avail AND a.product_quantity_avail < a.Quantity_sold * 0.70 THEN 'Medium inventory, need to add some inventory'
                WHEN a.product_quantity_avail >= a.Quantity_sold * 0.70 THEN 'Sufficient inventory' 
            END
...
FROM
    (SELECT 
     ...
     JOIN  
         product_class pc ON p.product_class_code = pc.product_class_code
     GROUP BY 
         p.product_id, p.product_desc, p.product_quantity_avail, pc.product_class_desc) a
ORDER BY 
    a.product_id;
